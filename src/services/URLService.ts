...
export class URLService {
    ...
    async shortenUrl(data: URLCreate): Promise<URLResponse> {
        try {
          const response = await this.client.post<URLResponse>('/', data);
    ...
    async redirectFromShortCode(shortCode: string): Promise<void> {
        try {
          await this.client.get(`/${shortCode}`);
        } catch (error) {
          throw error;
        }
    }
    ...
    async getUrlStats(shortCode: string): Promise<UrlStats> {
        try {
          const response = await this.client.get<UrlStats>(`/${shortCode}/stats`);
          return response.data;
        } catch (error) {
          throw error;
        }
    ...
    async getUserUrls(limit: number = 64, offset: number = 0): Promise<Pagination<URLResponse>> {
        try {
          const response = await this.client.get<Pagination<URLResponse>>('/user/url', {
            params: { limit, offset },
          });
          return response.data;
        } catch (error) {
    ...
    async deleteUserUrl(id: number): Promise<void> {
        try {
          await this.client.delete('/user/url', {
            data: { id },
          });
        } catch (error) {
          throw error;
    ...
    async setFavoriteUrl(urlId: number, isFavorite: boolean): Promise<void> {
        try {
            await this.client.put('/user/url/favorite', {
            url_id: urlId,
