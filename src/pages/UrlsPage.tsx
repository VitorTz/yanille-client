...
export default function UrlsPage() {
  ...
  const [selectedUrlForStats, setSelectedUrlForStats] =
    useState<URLResponse | null>(null);
  const [urlStats, setUrlStats] = useState<UrlStats | null>(null);
  const [loadingStats, setLoadingStats] = useState(false);

  const [selectedUrlForTag, setSelectedUrlForTag] = useState<number | null>(
    null
  );
  ...
  const getFilteredAndSortedUrls = () => {
    ...
    if (filterTags.length > 0) {
      filtered = filtered.filter(
        (url) => url.tags && url.tags.some((tag) => filterTags.includes(tag.id))
      );
    }
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case "alpha-asc":
          return (a.title || a.original_url).localeCompare(
            b.title || b.original_url
          );
        case "alpha-desc":
          return (b.title || b.original_url).localeCompare(
            a.title || a.original_url
          );
        default:
          return 0;
      }
  ...
  const handleRemoveTagFromUrl = async (urlId: number, tagId: number) => {
    try {
      ...
      setUrls(
        urls.map((url) => {
          return url.id === urlId
            ? { ...url, tags: url.tags.filter((tag) => tag.id !== tagId) }
            : url;
        })
      );
    } catch (error: any) {
  ...
  const handleShortenUrl = async () => {
    ...
    setLoading(true);
    try {
      const urlResponse: URLResponse = await api.url.shortenUrl({
        url: newUrl,
  ...
  const handleAddTagToUrl = async (urlId: number, tag: UrlTag) => {
    try {
      await api.tag.createUrlTagRelation(urlId, tag.id);
    ...
    <>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      {filterFavorites && <Check className="w-4 h-4" />}
                    </button>
                  </div>
            ...
            >
              <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
                    ...
                    <div className="flex flex-wrap gap-2 mt-3">
                      {url.tags.map((tag) => (
                        ...
                        >
                          ...
                          <button
                            onClick={() =>
                              handleRemoveTagFromUrl(url.id, tag.id)
                            }
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() =>
                      handleToggleFavorite(url.id, url.is_favorite === true)
                    }
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-600">
                      Add this URL to your favorites
                    </p>
                  </div>
                </div>
                <button
      ...
      {selectedUrlForStats && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-3xl max-h-[90vh] flex flex-col relative">
            {/* Header fixo */}
            <div className="flex items-center justify-between p-6 border-b border-slate-100 sticky top-0 left-0 right-0 bg-white z-20 rounded-t-2xl">
            ...
            <div className="overflow-y-auto p-6 space-y-4">
              ...
              {loadingStats ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm text-slate-600">Clicks Today</p>
                          <p className="text-2xl font-bold text-slate-900">
                            {urlStats.clicks_today}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl p-5 border border-slate-200 shadow-sm">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm text-slate-600">First Click</p>
                          <p className="text-sm font-medium text-slate-900">
                            {urlStats.first_click
                              ? new Date(
                                  urlStats.first_click
                                ).toLocaleDateString()
                              : "Never"}
                  ...
                  {/* Browsers */}
                  {urlStats.browsers && urlStats.browsers.length > 0 && (
                    <div className="bg-white rounded-xl p-5 border border-slate-200">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {urlStats.browsers.map((browser, index) => (
                          <span
                            key={index}
                            className="px-3 py-1 bg-slate-50 border border-slate-200 rounded-full text-sm text-slate-700"
                          >
                            {browser}
                          </span>
                        ))}
                  ...
                  {/* Operating Systems */}
                  {urlStats.operating_systems &&
                    urlStats.operating_systems.length > 0 && (
                      <div className="bg-white rounded-xl p-5 border border-slate-200">
                        <h4 className="font-semibold text-slate-900 mb-3 flex items-center gap-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {urlStats.operating_systems.map((os, index) => (
                            <span
                              key={index}
                              className="px-3 py-1 bg-slate-50 border border-slate-200 rounded-full text-sm text-slate-700"
                            >
                  ...
                  {/* Device Types */}
                  {urlStats.device_types &&
                    urlStats.device_types.length > 0 && (
                      <div className="bg-white rounded-xl p-5 border border-slate-200">
                        <h4 className="font-semibold text-slate-900 mb-3 flex items-center gap-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {urlStats.device_types.map((device, index) => (
                            <span
                              key={index}
                              className="px-3 py-1 bg-slate-50 border border-slate-200 rounded-full text-sm text-slate-700"
                            >
                  ...
                  <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-slate-900 font-medium">
                        {new Date(
                          selectedUrlForStats.created_at
                        ).toLocaleString()}
                      </span>
                    </div>
                  </div>
                ...
                <p>This URL has no statistics yet!</p>
              )}
            </div>
          </div>
        </div>
